...
/**
 * A list of `ZendriveTagInfo` for this trip.
 * This list will only be populated in `-[ZendriveDelegateProtocol processAnalysisOfDrive:]` callback.
 *
 * In case of no tags in the trip, an empty list is returned.
 *
 * @see `+[ZendriveVehicleTagging getAssociatedVehicleForDrive:]`
 *
 */
@property (nonatomic, strong, nonnull) NSArray<ZendriveTagInfo *> *tags;

/**
 * The type of vehicle.
 * If `driveType` is not `ZendriveDriveType.ZendriveDriveTypeDrive` then it
 * will be set to `ZendriveVehicleType.ZendriveVehicleTypeUnknown`.
 * NOTE: This value will only be populated in `-[ZendriveDelegateProtocol processAnalysisOfDrive:]` callback.
 */
